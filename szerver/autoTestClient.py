...
for i in range(1001):
	try:
		r = requests.post('http://localhost:5000',json={'uname':'TEST_NUMBER_' + str(i), 'het':'E', 'nap':'P', 'tant': hashlib.sha1(str(i).encode('utf-8')).hexdigest(), 'anyag': hashlib.md5(str(i).encode('utf-8')).hexdigest()})
		r.raise_for_status()
